...
export const load: PageServerLoad = async ({ params: { id }, locals, url }) => {
	...
	const user = await prisma.authUser.findFirst({ where: { id: id }, include: { teacher: true } });
	if (!user) {
		throw error(404, 'teacher not found.');
	}
	const form = await superValidate(user.teacher, schema);
	return { form };
};
...
export const actions: Actions = {
	default: async ({ request, params }) => {
		...
		try {
			const { name, degree, special } = form.data;
