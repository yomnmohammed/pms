<script lang="ts">
	...
	breadcrumb.set([{ label: 'لوحة التحكم' }, { label: 'استاذة', href: '/admin/teachers' }]);
			...
			{:then teachers}
				{#each teachers as teacher (teacher.id)}
					<tr>
						...
						<td>{teacher.teacher?.special ?? ''}</td>
						<td>{teacher.teacher?.degree ?? ''}</td>
						<td class="flex gap-2">
							<a href={`/admin/teachers/${teacher.id}/edit`}
								><Pencil class="h-5 w-5 fill-white stroke-green-500" /></a
							>
							<form
								action={`?/delete&id=${teacher.id}${calculateRedirectToTablePage()}`}
								method="POST"
								use:enhance={() => {
									return async ({ result }) => {
